...
VFH_Algorithm::SetCurrentMaxSpeed( int max_speed )
{
    if(max_speed > this->MAX_SPEED)
    	...
    	this->Current_Max_Speed = max_speed;
                               ...
                               
{
  ...
  printf("\n\n\n");
  printf ("Update \n");
  printf("60 : [%.2f], 70 : [%.2f], 80 : [%.2f], ", laser_ranges[120][0], laser_ranges[140][0], laser_ranges[160][0]);
  printf("90 : [%.2f], 100 : [%.2f], 110 : [%.2f]\n", laser_ranges[180][0], laser_ranges[200][0], laser_ranges[220][0]);
  ...
  printf("desired_angle : %.2f, d2g : %.2f, g_d : %.2f\n", goal_direction, goal_distance, goal_distance_tolerance);
  // 
  ...
  diff.tv_sec = now.tv_sec - last_update_time.tv_sec;
  diff.tv_usec = now.tv_usec - last_update_time.tv_usec;
  if(diff.tv_usec < 0){
	  --diff.tv_usec;
  ...
  printf("size : %d", sizeof(now.tv_usec));
  //aaa = (long int)now.tv_usec;
  ...
  printf("speed_incr : %d, pick : %d\n", speed_incr, Max_Speed_For_Picked_Angle);
  // Accelerate (if we're not already at Max_Speed_For_Picked_Angle).
  if(last_chosen_speed + speed_incr > Max_Speed_For_Picked_Angle )
  {
	  printf("why last : %d, spdin : %d, pick : %d\n", last_chosen_speed, speed_incr, Max_Speed_For_Picked_Angle);
...
//  chosen_speed = MIN( last_chosen_speed + speed_incr, Max_Speed_For_Picked_Angle );
 ...
 // printf("CHOSEN: SPEED: %d\t TURNRATE: %d\n", chosen_speed, chosen_turnrate);
  ...
  printf("AFTER CHOSEN: SPEED: %d\t TURNRATE: %d\n", chosen_speed, chosen_turnrate);
...
bool VFH_Algorithm::Cant_Turn_To_Goal()
{
    ...
    float goal_x = this->Dist_To_Goal * cos( static_cast<float> ((this->Desired_Angle)*M_PI/180) );
    float goal_y = this->Dist_To_Goal * sin( static_cast<float> ((this->Desired_Angle)*M_PI/180) );
...
//  printf("Start: %d\n", start);
 ...
 //   printf("angle : %d \n", angle);
    ...
    if (fabs(angle) < 80) 
    {
        ...
        if(Current_Max_Speed < MAX_SPEED_NARROW_OPENING)
        	Candidate_Speed.push_back(MAX_SPEED_NARROW_OPENING);
        else
    ...
    else 
    {
        ...
        Candidate_Angle.push_back(new_angle);
        if(Current_Max_Speed > MAX_SPEED_WIDE_OPENING)
        	Candidate_Speed.push_back(MAX_SPEED_WIDE_OPENING);
        else
        	Candidate_Speed.push_back(Current_Max_Speed);
...
int VFH_Algorithm::Build_Primary_Polar_Histogram( double laser_ranges[361][2], int speed ) 
{
 ...
 // printf("60 : [%.2f], 70 : [%.2f], 80 : [%.2f]", laser_ranges[120][0], laser_ranges[140][0], laser_ranges[160][0]);
 // printf("90 : [%.2f], 100 : [%.2f], 110 : [%.2f]", laser_ranges[180][0], laser_ranges[200][0], laser_ranges[220][0]);

  if ( Calculate_Cells_Mag( laser_ranges, speed ) == 0 )
  {
      printf(" set Hist to all blocked");
      for(x=0;x<HIST_SIZE;x++) {
          Hist[x] = 1;
      }
      return 0;
...
int VFH_Algorithm::Build_Masked_Polar_Histogram(int speed) 
{
  ...
  int obstacle_left_end,obstacle_right_end;  
...
int VFH_Algorithm::Set_Motion( int &speed, int &turnrate, int actual_speed ) 
{
  ...
  else 
  {
    ...
    if ((Picked_Angle > 270) && (Picked_Angle < 360)) {
	  ...
	  //printf("picked angle : %f    ", Picked_Angle);
